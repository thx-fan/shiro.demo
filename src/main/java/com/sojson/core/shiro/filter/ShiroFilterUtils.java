...
public class ShiroFilterUtils {
	...
	private boolean isUrlExcluded(String url) {
		if (StringUtils.isNotBlank(url)) {
			if (url.equals("/swagger-ui.html") || isStaticResources(url) || isRestful(url) || url.contains("404")) {
				return true;
			}
		}
		return false;
	...
	private boolean isStaticResources(String url) {
		return url.endsWith(".css") || url.endsWith(".js") || url.endsWith(".map") || url.endsWith(".ttf") || url.endsWith(".png") || url.endsWith(".gif");
	}

	private boolean isRestful(String url) {
		return url.startsWith("/swagger-resources")  || url.startsWith("/v2/api-docs");
	}
}
